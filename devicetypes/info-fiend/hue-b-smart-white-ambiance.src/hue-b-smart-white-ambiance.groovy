 ...
 *	Version 1.1 -- included TMLeafs edits
 * 
...
def off() {
	if(device.currentValue("idelogging") == "All" || device.currentValue("idelogging") == "OnOff") {
		log.trace "Hue B Smart Ambience Bulb: off(): "
    }
...
def poll() {
	if(device.currentValue("idelogging") == 'All'){
		log.debug "Hue B Smart Ambience Bulb: poll()."	
    }
    refresh()
}
...
def reset() {
	if(device.currentValue("idelogging") == 'All' || device.currentValue("idelogging") == "OnOff") {
		log.trace "Hue B Smart Ambience Bulb: reset(): "
    }
