	...
	tiles (scale: 2){
		...
		standardTile("flash", "device.flash", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
 		...
 		standardTile("flash_on", "device.flash_on", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
			state "default", label:"Flash on", action:"flash_on", icon:"st.lights.philips.hue-multi"
		}
...
def setTransitionTime(tt) {
	log.trace "Hue B Smart Gr oup-: setTransitionTime(): "
    if (tt == null) { tt = 4 }
...
def setTT3sec() {
	log.trace "Hue B Smart Group: setTT3sec(): "
...
def flash_on() {
	log.trace "Hue B Smart Group: flash_on(): "
    def commandData = parent.getCommandData(device.deviceNetworkId)
	parent.sendHubCommand(new physicalgraph.device.HubAction(
    	[
        	method: "PUT",
